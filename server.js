...
async function run() {
    ...
    setInterval(async () => {
        ...
        const format = (date) => date.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }).toUpperCase().replace(/\./g, '');
        ...
        upcoming.forEach(m => io.to(m._id.toString()).emit("oneMinWarning", { msg: `আপনার ম্যাচ ১ মিনিট পর শুরু হবে!` }));
